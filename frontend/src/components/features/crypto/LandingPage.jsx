...
const LandingPage = () => {
  ...
  const getSpecificCoins = () => {
    ...
    // Log để debug
    console.log('Available coins:', coins.map(c => c.symbol?.toLowerCase()));
    console.log('Filtered coins:', filteredCoins.map(c => c.symbol?.toLowerCase()));
  ...
  const getPopularCoins = () => {
    const popularSymbols = ["btc", "eth", "usdt", "bnb", "sol"];
    return coins.filter(coin => 
      popularSymbols.includes(coin.symbol?.toLowerCase())
    );
  };
